...
export function SelectedWork() {
  ...
  return (
    <section id="work" className="relative px-5 pb-24 pt-10 md:px-10 lg:px-14">
      <div className="mx-auto max-w-[1280px]">
        <header className="mb-10 flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-[11px] tracking-[0.28em] text-white/40">SELECTED WORK</p>
            <h2 className="mt-2 text-[clamp(1.8rem,3.2vw,2.8rem)] font-medium tracking-tight text-white">
              精选项目
            </h2>
          </div>
          <Link
            to="/work"
            className="group inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/[0.04] px-4 py-2 text-[12px] tracking-wide text-white/70 transition hover:border-white/25 hover:text-white"
          >
            View all
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 md:gap-4">
          {selectedFive.map((item) =>
            item.slug === active?.slug ? (
              <BorderGlow key={item.slug} className="relative min-w-[min(100%,520px)] flex-1 overflow-hidden rounded-[32px]">
                <article className="relative flex h-[420px] flex-col overflow-hidden rounded-[32px] border border-white/10 bg-[#0b0d14]/90">
                  <div className="relative z-[2] flex items-start justify-between gap-4 px-6 pt-6">
                    <div>
                      <p className="text-[11px] tracking-[0.22em]" style={{ color: item.accent }}>
                        {item.kicker}
                      </p>
                      <h3 className="mt-1.5 text-[1.55rem] font-medium tracking-tight text-white">
                        {item.title}
                      </h3>
                      <p className="mt-1 text-[12px] tracking-[0.18em] text-white/45">{item.lane}</p>
                    </div>
                    <span
                      className="rounded-full px-3 py-1 text-[11px] font-medium tracking-widest"
